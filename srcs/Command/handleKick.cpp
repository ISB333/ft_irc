...
void Handler::handleKick(Client  *client, const vector<std::string> &argv) {
    ...
    catch (const out_of_range&) { client->appendOutput(formatReply(ERR_NOSUCHCHANNEL, client->getNickname(), argv[0], "No such channel"));
        return;
    };
    if (not channel->isOperator(client->getFd())) {
        client->appendOutput(formatReply(ERR_CHANOPRIVSNEEDED, client->getNickname(), argv[0], "You're not channel operator"));
        return;
    };
};